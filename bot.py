...
# MongoDB connection - will be initialized later
mongo_client = None
db = None
...
async def get_custom_products():
    ...
    try:
        ...
        # Filter out products with invalid keys (too long or special chars)
        valid_products = []
        for c in custom:
            key = c.get("key", "")
            if key and len(key) <= 32 and key.isascii():
                valid_products.append({"name": c["name"], "key": key})
            elif key:
                # Generate a safe key from the name
                safe_key = "".join(c for c in key if c.isalnum() or c == "_")[:32]
                if safe_key:
                    valid_products.append({"name": c["name"], "key": safe_key})
...
async def admin_add_product_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create safe ASCII key from product name
    import re
    # Remove non-alphanumeric characters and replace spaces with underscores
    product_key = re.sub(r'[^a-zA-Z0-9_]', '', product_name.replace(" ", "_").lower())
    if not product_key:
        # If name is all non-ASCII (e.g., Arabic), generate a random key
        import random
        import string
...
async def approve_recharge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Error finding recharge request: {e}")
...
def main():
    ...
    # These must be added before regular CallbackQueryHandlers
    ...
    # Add conversation handlers FIRST (order matters!)
